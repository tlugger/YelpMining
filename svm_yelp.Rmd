...
getAccuraciesFromTable <- function(truth_table){
  ...
  for(truth in  1:5){
    ...
    for(predict in 1:5){
      ...
      if(truth == predict){
          accuratelyIdentified = result[truth, predict]
      }
      
      else{
        falsleyIdentified = result[truth, predict]
      }
...
accuracyChangeSVM <- function(sampleSize, fixedTestSize){
  allAccuracies <- c()
  for(i in seq(1, sampleSize, by = 1000)){
...
test <- accuracyChangeSVM(15000, 2000)
```


```{r}
svmMatrix <- matrix(test, nrow = 9, ncol = 5)
```
```{r}
svmMatrix
```
